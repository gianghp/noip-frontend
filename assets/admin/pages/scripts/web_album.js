var WebAlbum = function () {
    var initGrid = function () {
        ...
        grid.init({
            ...
            dataTable: {
                ...
                stateSave: true,
                ajax: {
                    url: "/admin/web_album/get.do?ngay=" + $('#thoi_gian input').val() + "&pattern=" + $('#frmGridSearch').find("input").val()
                },
                columnDefs: [
                {
                    orderable: false,
    ...
    var fnDelete = function(o){
        if (o === false){
            if (WebAlbum.grid.getSelectedRowsCount() === 0)
                ...
                bootbox.dialog({
                    ...
                    buttons: {
                        success: {
                            ...
                            callback: function () {
                                $.ajax({
                                    url : "/admin/web_album/delete.do",
                                    data : {id: WebAlbum.grid.getSelectedRows(), album_id : Metronic.getURLParameter("album_id")},
                                    dataType : 'json',
                                    type : 'post',
                                    success: function(d)
                                    {
                                        if (!d.success) Metronic.showMessage('Thông báo', 'Bạn không có quyền thực hiện chức năng này.', 'OK');
                                        WebAlbum.grid.getDataTable().ajax.reload();
                                    }
                                });
                            }
                        },
    ...
    var fnAdd = function(document_id){
        $.ajax({
            url : "/admin/web_album/getEditForm.do",
            ...
            success : function(data){
                ...
                html.find('#btn_files').click(function(){
                    Metronic.showUpload("Ảnh đại diện", 2, function(a, b){
                        var img = {url : '', name : ''};
                        var table = html.find('#list');
                        $(a).each(function(){
                            if (img.url === '') img = this;
                            var tplRow = $(html.find('#tplRow tbody').html());
                            tplRow.find('#image_url').val(this.url);
                            tplRow.find('#imagethumb').attr('src', this.url);
                            tplRow.find('#name').val(this.name);
                            table.append(tplRow);
                        });
                        
                        html.find('#imgThumbAlbum').attr('src', img.url);
                        html.find('#image_url').val(img.url);
                    });
                });
                
                html.find('#btn_files2').click(function(){
                    Metronic.showUpload("Tải ảnh lên", 2, function(a, b){
                        var img = {url : '', name : ''};
                        var table = html.find('#list');
                        $(a).each(function(){
                            if (img.url === '') img = this;
                        ...
                        if (html.find('#imgThumbAlbum').attr('src') === '/assets/images/noimage.png'){
                            html.find('#imgThumbAlbum').attr('src', img.url);
                            html.find('#image_url').val(img.url);
                        }
                    });
                    ...
                    buttons: {
                        success: {
                            ...
                            callback: function () {
                                ...
                                if (subject.length === 0){
                                    Metronic.showMessage("Thông báo", "Bạn chưa nhập tiêu đề album.", "OK");
                                    return false;
                                }
                                
                                html.parents('.modal-content').mask("Đang thực hiện...");
                                $.ajax({
                                    url : "/admin/web_album/add.do",
                                    dataType : "json",
                                    ...
                                    success : function(d){
                                        ...
                                        if (d.success){
                                            box.modal('hide');
                                            if ($('#panel_album_info').length === 0){
                                                Metronic.showMessage("Thông báo", "Đã thêm album thành công!", "OK");
                                            }
                                            else WebAlbum.grid.getDataTable().ajax.reload();
                                        }
                ...
                <div style='float:left;'><input class='check' id='hotalbum' type='checkbox'/> Tin nóng</div>");
    ...
    var fnEdit = function(o){
        $.ajax({
            url : "/admin/web_album/getEditForm.do?id=" + o.id,
            dataType : "json",
            success : function(data){
                var html = $(data.html);
                    ...
                    buttons: {
                        success: {
                            ...
                            callback: function () {
                                ...
                                if (content.length === 0){
                                    Metronic.showMessage("Thông báo", "Bạn chưa nhập nội dung album.", "OK");
                                    return false;
                                }

                                html.parents('.modal-content').mask("Đang thực hiện...");
                                $.ajax({
                                    url : "/admin/web_album/save.do",
                                    dataType : "json",
                                    data : html.serializeArray(),
                                    success : function(d){
                                        html.parents('.modal-content').unmask();
                ...
                if (html.find('#hotalbum').val() == 1){
                    checkpn.find('#hotalbum').attr('checked', 'checked');
                    checkpn.find('#hotalbum').change();
                }
