...
var Metronic = function () {
    ...
    return {
        ...
        init: function () {
            ...
            $('.page-sidebar-wrapper li a').click(function () {
                $.createCookie("menu_click", 1);
            });
        },
        ...
        notificationHtml5: function (theTitle, theBody, theIcon, timeout, theSound, data, fn) {
            var options = {body: theBody, noscreen: true, data : data, lang : 'vn-VN'};
            ...
            if (theSound !== undefined){
                $('#playSound').remove();
                $('body').append('<audio id="playSound" autoplay src="' + theSound + '"></audio>');
            }
        ...
        notification: function (title, text, image, timeout, audio, data, fn)
        {
            image = image === undefined ? "resources/images/no-avatar.jpg" : image;
            timeout = timeout === undefined || timeout === 0 ? 7000 : timeout;
            audio = audio === undefined ? 'assets/sounds/02.mp3' : (audio === '' ? undefined : audio);
            if (!this.notificationHtml5(title, text, image, timeout, audio, data, fn)){
        ...
        showUpload: function (title, ext, fnUpload)
        {
                ...
                buttons: {
                    success: {
                        ...
                        callback: function () {
                            ...
                            if (fnUpload)
                                return fnUpload(aFiles, files, box);
                            else
                                return true;
                            $('#fileupload_').remove();
                        }
            ...
            $('#fileupload_').fileupload({
                ...
                acceptFileTypes: ext === 1 ? /(\.|\/)(gif|jpg|jpe?g|png?)$/i : /(\.|\/)(gif|jpg|jpe?g|png|xls|xlsx|rar|zip|doc|docx|ppt|pptx|mp3|wav|psd|pdf|sql|swf|txt|gz|dwg|dwt|dws|mp4|flv|mov)$/i
            });
        },
